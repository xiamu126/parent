...
public interface IRegionController {
    ...
    // 获取某个监测点的AQI（日报）
    @GetMapping(value="report/aqi/day/{deviceId}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    AQIResult getAQILastDayOfDevice(@PathVariable(name = "deviceId") Integer deviceId);

    // AQI历史数据按小时来
    @GetMapping(value="report/aqi/hourly/{deviceId}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    List<AQIResult> getAQIHourlyOfDevice(@PathVariable(name = "deviceId") Integer deviceId);
    // AQI历史数据按天来
    @GetMapping(value="report/aqi/daily/{deviceId}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    List<AQIResult> getAQIDailyOfDevice(@PathVariable(name = "deviceId") Integer deviceId);
    // AQI历史数据按月来
    @GetMapping(value="report/aqi/monthly/{deviceId}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    List<AQIResult> getAQIMonthlyOfDevice(@PathVariable(name = "deviceId") Integer deviceId);

    // AQI历史数据按小时来，指定时间区间
    @GetMapping(value="report/aqi/hourly/{deviceId}/{begin}/{end}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    ...
    // AQI历史数据按天来，指定时间区间
    @GetMapping(value="report/aqi/daily/{deviceId}/{begin}/{end}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    List<AQIResult> getAQIDailyOfDeviceBetween(@PathVariable(name = "deviceId") Integer deviceId,
                                               @PathVariable(name = "begin") Long begin,
                                               @PathVariable(name = "end") Long end);
    // AQI历史数据按月来，指定时间区间
    @GetMapping(value="report/aqi/monthly/{deviceId}/{begin}/{end}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    List<AQIResult> getAQIMonthlyOfDeviceBetween(@PathVariable(name = "deviceId") Integer deviceId,
                                                 @PathVariable(name = "begin") Long begin,
                                                 @PathVariable(name = "end") Long end);

    // 元素历史数据按小时来
    @GetMapping(value="report/hourly/{deviceId}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    Map<String, List<AVGResult>> getAvgHourlyOfDevice(@PathVariable(name = "deviceId") Integer deviceId);
    // 元素历史数据按天来
    @GetMapping(value="report/daily/{deviceId}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    Map<String, List<AVGResult>> getAvgDailyOfDevice(@PathVariable(name = "deviceId") Integer deviceId);
    // 元素历史数据按月来
    @GetMapping(value="report/monthly/{deviceId}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    Map<String, List<AVGResult>> getAvgMonthlyOfDevice(@PathVariable(name = "deviceId") Integer deviceId);

    // 元素历史数据按小时来
    @GetMapping(value="report/hourly/{deviceId}/{begin}/{end}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    Map<String, List<AVGResult>> getAvgHourlyOfDeviceBetween(@PathVariable(name = "deviceId") Integer deviceId,
                                                             @PathVariable(name = "begin") Long begin,
                                                             @PathVariable(name = "end") Long end);
    // 元素历史数据按天来
    @GetMapping(value="report/daily/{deviceId}/{begin}/{end}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
    Map<String, List<AVGResult>> getAvgDailyOfDeviceBetween(@PathVariable(name = "deviceId") Integer deviceId,
                                                            @PathVariable(name = "begin") Long begin,
                                                            @PathVariable(name = "end") Long end);
    // 元素历史数据按月来
    @GetMapping(value="report/monthly/{deviceId}/{begin}/{end}", produces = {MediaType.APPLICATION_JSON_UTF8_VALUE})
