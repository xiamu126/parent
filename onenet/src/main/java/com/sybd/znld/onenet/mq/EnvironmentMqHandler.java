...
import com.sybd.znld.onenet.websocket.handler.EnvironmentWsHandler;
...
@Slf4j
@Component
public class EnvironmentMqHandler {
    private final ObjectMapper objectMapper;
    private final IMessageService messageService;

    @Autowired
    public EnvironmentMqHandler(ObjectMapper objectMapper, IMessageService messageService) {
    ...
    public void handler(Channel channel, Message message) {
        try {
            ...
            if (news != null) {
                EnvironmentWsHandler.sendAll(this.objectMapper.writeValueAsString(news)); // 推送消息
            }
