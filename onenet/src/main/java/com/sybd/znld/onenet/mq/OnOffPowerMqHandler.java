...
import com.sybd.znld.onenet.websocket.handler.OnOffPowerWsHandler;
...
@Slf4j
@Component
public class OnOffPowerMqHandler {
    private final ObjectMapper objectMapper;
    private final IMessageService messageService;

    @Autowired
    public OnOffPowerMqHandler(ObjectMapper objectMapper, IMessageService messageService) {
        this.objectMapper = objectMapper;
        this.messageService = messageService;
    }

    @Async("MqThreadPool")
    @RabbitListener(queues = IOneNetService.ONENET_ONOFF_UP_QUEUE)
    public void handler(Channel channel, Message message) {
        try {
            ...
            if(news != null) {
                OnOffPowerWsHandler.sendAll(this.objectMapper.writeValueAsString(news)); // 推送消息
            }
