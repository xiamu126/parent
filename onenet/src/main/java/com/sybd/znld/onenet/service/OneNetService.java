...
public class OneNetService implements IOneNetService {
    ...
    public OneNetService(LampMapper lampMapper,
                         ...
                         DataLocationMapper dataLocationMapper, DataAngleMapper dataAngleMapper) {
    ...
    @Override
    public boolean addOrModifyDeviceOnOff(DataPushModel model) {
        var ret = this.dataDeviceOnOffMapper.selectByDeviceIdAndResourceName(model.deviceId, model.name);
        if(ret == null){ // 这个设备的此开关状态没有保存过
            return this.dataDeviceOnOffMapper.insert(model) > 0;
        }else{
            return this.dataDeviceOnOffMapper.updateByDeviceIdAndResourceName(model) > 0;
        }
    }

    @Override
    public boolean addOrModifyDeviceLocation(DataPushModel model) {
        var ret = this.dataLocationMapper.selectByDeviceIdAndResourceName(model.deviceId, model.name);
    ...
    @Override
    public boolean addOrModifyDeviceAngle(DataPushModel model) {
        var ret = this.dataAngleMapper.selectByDeviceIdAndResourceName(model.deviceId, model.name);
        if(ret == null){ // 这个设备的倾斜没有保存过
            return this.dataAngleMapper.insert(model) > 0;
        }else{
            return this.dataAngleMapper.updateByDeviceIdAndResourceName(model) > 0;
        }
    }
