...
public class VideoAsyncTask {
    ...
    private static ConcurrentHashMap<String, String> threadPools = new ConcurrentHashMap<>();
    ...
    public void stopAll(){
        for(var it : threadPools.entrySet()){
            var channelGuid = it.getKey();
            stopRedis(channelGuid);
        }
        threadPools.clear();
        try {
    ...
    public boolean isChannelInUsing(String channelGuid){
        return threadPools.containsKey(channelGuid);
    }
    ...
    public void push(String channelGuid, String rtspPath, String rtmpPath, int audioRecord) {
        ...
        try {
            ...
            recorder.start();
            while(threadPools.containsKey(channelGuid)){
                var frame = grabber.grabImage();
