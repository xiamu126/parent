...
create table authority_group(
  ...
  parent_id            varchar(32) default 0 comment '上级权限模块编号，0为顶级模块',
  position             int unsigned default 0 comment '在当前权限模块层级中的位置，0为第一个位置',
  status               tinyint unsigned default 0 comment '0：正常，1：冻结'
);
...
create table authority(
  ...
  type                 int unsigned default 0 comment '0：菜单，1：按钮，2：其他',
  status               tinyint unsigned default 0 comment '0：正常，1：冻结，2：删除'
);
...
create table user(
  ...
  gender            bit default 3 comment '0为女，1为男，3为未知',
  age               tinyint unsigned default 0,
  contactAddress    varchar(64) default '' comment '联系地址',
  realName          varchar(10) default '' comment '真实姓名',
  idCardNo          varchar(18) default '' comment '身份证号',
...
create table organization(
  ...
  parent_id            int unsigned default 0 comment '上级组织编号，0为顶级组织',
  position             tinyint unsigned default 0 comment '在当前组织层级的位置',
  status               tinyint unsigned default 0 comment '0：正常，1：冻结，2：删除',
...
create table role(
  ...
  type                 tinyint unsigned default 0 comment '0：管理员，1：普通用户，2：游客',
  status               tinyint unsigned default 0 comment '0：可用，1：冻结'
);
